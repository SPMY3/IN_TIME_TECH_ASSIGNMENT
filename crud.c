...
struct User { int id; char name[50]; int age; };

void addUser() {
    struct User u; FILE *f=fopen("users.txt","a");
    ...
    fprintf(f,"%d %s %d\n",u.id,u.name,u.age);
    fclose(f); printf("User added!\n");
}

void showUsers() {
    struct User u; FILE *f=fopen("users.txt","r");
    if(!f){ printf("No users found\n"); return; }

    printf("ID\tName\tAge\n");
    while(fscanf(f,"%d %s %d\n",&u.id,u.name,&u.age)!=EOF) printf("%d\t%s\t%d\n",u.id,u.name,u.age);
    fclose(f);
}

void updateUser() {
    int id,found=0; struct User u;
    FILE *f=fopen("users.txt","r"), *t=fopen("temp.txt","w");
    if(!f||!t){ printf("Error opening file\n"); return; }

    printf("Enter ID to update: "); scanf("%d",&id); getchar();
    while(fscanf(f,"%d %s %d\n",&u.id,u.name,&u.age)!=EOF){
        if(u.id==id){ found=1; printf("Enter new Name: "); fgets(u.name,50,stdin); u.name[strlen(u.name)-1]='\0';
